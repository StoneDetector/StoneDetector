  ...
  */
	...
	//allow empty identifier because they are sometimes used.
		if (!simplename.matches("<.*>|\\d.*|^.{0}$")) {
			...
			if (checkAllParts(splittedSimplename)) {
				throw new SpoonException("No allowed javaletter or keyword in identifier found. See JLS for correct identifier. Identifier: #" + simplename+"#");
			}
	}
}
	...
	private boolean checkAllParts(String[] simplenameParts) {
		for (String simpleName:simplenameParts) {
			...
			if (simpleName.equals("_"))
				;
			else if (isKeyword(simpleName) || checkIdentifierChars(simpleName)) {
				return true;
		}
	}
	return false;
	...
	"transient", "catch", "extends", "try", "final", "interface", "static", "finally",  "strictfp", "volatile",
			 "const",  "native", "super", "while")//, "_") // I have changed this. Check this. WA
