...
public abstract class Dominators<T> {
  ...
  public AbstractGraph<T> dominatorTree() {
    return new AbstractGraph<T>() {
      ...
      private final EdgeManager<T> edges =
		...
		if(getIdom(N) == null){
		    //Assertions.UNREACHABLE("There is a node, which has no dominator, but only root can have this. That must be fixed H " + N);
		    return null;
		}
		else
	      ...
	      //@Override
            public boolean removeEdge(Object src, Object dst) {
              Assertions.UNREACHABLE();
	      return false;
            }
