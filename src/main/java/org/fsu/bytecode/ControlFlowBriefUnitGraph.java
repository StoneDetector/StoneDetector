...
public class ControlFlowBriefUnitGraph extends BriefUnitGraph implements Graph<Unit>
{
    public Unit headNode;
    public int exceptionCount;

    public ControlFlowBriefUnitGraph(Body body, FileBasedConfiguration configuration)
    {
        super(body);
        ...
        if (configuration.getBoolean("connectPathBriefUnitGraph")){
            headNode = this.unitChain.getFirst();

            for (Unit unit : this.unitChain) {
                if (getPredNodeCount(unit) == 0){
                    if (!unit.equals(headNode)){
                        super.addEdge(this.unitToSuccs, this.unitToPreds, headNode, unit);
                    }
                }
            }
        }
